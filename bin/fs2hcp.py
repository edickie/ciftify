...
for Hemisphere in ['L', 'R']:
  ...
  for Surface in ['sphere.reg', 'sphere']:
    run(['mris_convert', os.path.join(FreeSurferFolder,'surf','{}h.{}'.format(hemisphere,Surface),
      os.path.join(AtlasSpaceFolder,NativeFolder,'{}.{}.{}.native.surf.gii'.format(Subject, Hemisphere, Surface))])
    run(['wb_command', '-set-structure',
      os.path.join(AtlasSpaceFolder,NativeFolder,'{}.{}.{}.native.surf.gii'.format(Subject, Hemisphere, Surface)),
      Structure, '-surface-type', 'SPHERICAL'])
  run(['wb_command', '-add-to-spec-file',
    os.path.join(AtlasSpaceFolder,NativeFolder,'{}.native.wb.spec'.format(Subject),
    Structure, os.path.join(AtlasSpaceFolder,NativeFolder,'{}.{}.sphere.native.surf.gii'.format(Subject, Hemisphere))])
