...
"""
write parcellated data to a pscalar.nii file

Usage:
    ciftify_write_pscalar <parcellated_result.csv> <parcel_template.dlabel.nii> <output.pscalar.nii> [options]

Arguments:
    <parcellated_result.csv>      The results to write to a pscalar file
    <parcel_template.dlabel.nii>  The dlabel file with the location of parcels to write to
    <output.pscalar.nii>          Output pscalar filename

Options:
  --header-row                    Indicates that the first row of the parcellated result contains columnames
  --labeltable CSV                Optional label table to check against/align with the dlabels labels
  --debug                         Debug logging
  -h,--help                       Print help

Details:

Will write the data from a text file of results to a pscalar.nii output file for visualization with wb_command
